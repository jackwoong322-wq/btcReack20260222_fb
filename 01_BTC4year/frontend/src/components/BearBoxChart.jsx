...
export default function BearBoxChart({ cycleNumber = 4 }) {
  ...
  useEffect(() => {
    ...
    // ── 예측 박스 (Cycle 4 전용) ────────────────────────────────
    if (cycleNumber === 4 && predictions.length > 0) {
    ...
    if (cycleNumber === 4 && predictions.length > 0 && chartData.length > 0) {
  ...
  }, [lineData, boxes, predictions, cycleNumber])
          ...
          <div className="chart-header">
            <div>
              ...
              <p style={{ color: '#848E9C', margin: '4px 0 0', fontSize: '12px' }}>
                ...
                {cycleNumber === 4 && predictions.length > 0 && (
