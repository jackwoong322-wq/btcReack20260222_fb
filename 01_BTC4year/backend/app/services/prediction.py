...
def calculate_bear_prediction(
    ...
    cycle4_boxes: List[Dict],
    cycle4_line: List[Dict],
) -> List[Dict[str, Any]]:
    """
    Cycle 1~3의 박스 패턴을 분석하여 Cycle 4 예측 박스를 생성

    Args:
        all_cycle_boxes: {1: [...], 2: [...], 3: [...]}
        cycle4_boxes: Cycle 4 실제 박스
        cycle4_line: Cycle 4 lineData (타임스탬프 참조용)
    ...
    # ── [개선②] 유사 사이클 → 가중치 조정 ────────────
    similar = find_most_similar_cycle(all_cycle_boxes, cycle4_boxes)
    ...
    # ── 절대 기준점 계산 ──────────────────────────────
    last_real_box = cycle4_boxes[-1] if cycle4_boxes else None
    ...
    start_box_num = len(cycle4_boxes) + 1
