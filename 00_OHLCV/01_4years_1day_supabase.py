...
# --- 환경 변수 로드 (00_OHLCV → 저장소 루트 → 02_backend/.env) ---
_REPO_ROOT = Path(__file__).resolve().parent.parent
env_path = _REPO_ROOT / "02_backend" / ".env"
...
def db_timestamp_to_ms(val) -> Optional[int]:
    """alt_cycle_data.timestamp (timestamptz → ISO 문자열) 등 → epoch ms."""
    if val is None:
        return None
    s = str(val).strip()
    try:
...
def get_last_saved_info(supabase: Client):
    ...
    if response.data and len(response.data) > 0:
        ...
        ms = db_timestamp_to_ms(timestamp_str)
        if ms is not None:
            return ms, timestamp_str
    return None, None
...
def get_cycle4_peak_info(supabase: Client):
    ...
    if response.data and len(response.data) > 0:
        row = response.data[0]
        ms = db_timestamp_to_ms(row["timestamp"])
        if ms is not None and row.get("close_price") is not None:
            return ms, row["close_price"]
    return None, None


def _date_str_to_ms(date_val) -> int:
    """public.ohlcv.date → UTC 자정 기준 epoch ms."""
    s = str(date_val)[:10]
...
def get_ohlcv_data(supabase: Client, from_timestamp_ms=None):
    """public.ohlcv에서 OHLCV 조회 (coin_id 필터, date 기준)."""
    query = (
    ...
    if from_timestamp_ms:
        from_date = datetime.fromtimestamp(
            from_timestamp_ms / 1000, tz=timezone.utc
        ).strftime("%Y-%m-%d")
    ...
    df["timestamp"] = df["date"].apply(_date_str_to_ms)
...
def save_full_data(supabase: Client, result_df):
    ...
    # 기존 데이터 삭제
    for cycle_num in [1, 2, 3, 4]:
        supabase.table(CYCLE_TABLE_NAME).delete().eq("coin_id", CYCLE_COIN_ID).eq(
            "cycle_number", cycle_num
        ).execute()
    ...
    for i in range(0, len(records), batch_size):
        ...
        supabase.table(CYCLE_TABLE_NAME).upsert(
            batch, on_conflict="coin_id,cycle_number,days_since_peak"
        ).execute()
...
def save_incremental_data(supabase: Client, df):
    ...
    supabase.table(CYCLE_TABLE_NAME).upsert(
        records, on_conflict="coin_id,cycle_number,days_since_peak"
    ).execute()
